...
{
  echo'<script>alert("Email and password can not be left blank");window.location.href="http://localhost/homeinfo/ucheck.php";</script>';
}else{
      $conn=my_connect();
      ...
      if(isset($validuser))
      {
        $_SESSION["validuser"]=$validuser;
         mysqli_close($conn);
          echo'<script>alert("Welcome");window.location.href="http://localhost/homeinfo/main.php";</script>';
          }else{
            mysqli_close($conn);
...
<div class="container-fluid bg">
  ...
  <label id="sign">Does not have a Account?<a href="signup.php"> Signup </a></label>
</form>
    </div>
    <div class="col-md-4 col-sm-4 col-xs-12"></div>
    </div>
